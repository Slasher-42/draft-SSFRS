...
export default function AdminOverviewPage() {
  ...
  useEffect(() => {
    ...
    api.get<User[]>("/api/admin/users").then((res) => setUsers(res.data)).finally(() => setLoading(false));
  }, []);
  ...
  const stats = [
    { label: t("totalUsers"), value: loading ? "—" : total, sub: t("totalUsersSub"), icon: <Users className="h-5 w-5" />, iconColor: "#3B82F6", iconBg: "#EFF6FF" },
    { label: t("activeUsers"), value: loading ? "—" : active, sub: t("activeUsersSub"), icon: <UserCheck className="h-5 w-5" />, iconColor: "#22c55e", iconBg: "#f0fdf4" },
    { label: t("lockedAccounts"), value: loading ? "—" : locked, sub: t("lockedAccountsSub"), icon: <Lock className="h-5 w-5" />, iconColor: "#ef4444", iconBg: "#fef2f2" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem", maxWidth: 1100 }}>
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        style={{ borderRadius: 18, padding: "2rem 2.5rem", minHeight: 168, position: "relative", overflow: "hidden", background: "linear-gradient(135deg, #451a03 0%, #92400e 55%, #d97706 100%)" }}>
      ...
      <div>
        <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.13em", color: "var(--color-muted-foreground)", marginBottom: "0.75rem" }}>{t("platformStats")}</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem" }}>
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 + i * 0.07 }} style={{ ...CARD_STYLE, padding: "1.25rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "0.875rem" }}>
                <div style={{ height: 40, width: 40, borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: s.iconBg, color: s.iconColor, flexShrink: 0 }}>{s.icon}</div>
                <span style={{ fontSize: "2.25rem", fontWeight: 900, color: s.iconColor, lineHeight: 1 }}>{s.value}</span>
              </div>
              ...
              {!loading && total > 0 && (
                <div style={{ marginTop: "0.875rem", height: 4, borderRadius: 999, backgroundColor: "var(--color-border)", overflow: "hidden" }}>
                  <motion.div initial={{ width: 0 }} animate={{ width: `${Math.round((Number(s.value) / total) * 100)}%` }} transition={{ duration: 0.7, ease: "easeOut", delay: 0.2 + i * 0.07 }} style={{ height: "100%", borderRadius: 999, backgroundColor: s.iconColor }} />
                </div>
              )}
            </motion.div>
          ))}

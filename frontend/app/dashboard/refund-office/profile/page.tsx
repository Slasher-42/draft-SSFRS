...
export default function RefundOfficeProfilePage() {
  const formId = useId();
  const [session, setSession] = useState<{ userId: string; role: string } | null>(null);

  const [user, setUser] = useState<UserProfile | null>(null);
  const [profile, setProfile] = useState<RefundOfficeProfile>(EMPTY_REFUND);
  ...
  useEffect(() => {
    ...
    userService
      ...
      .then((p) => setProfile(p as RefundOfficeProfile))
      .catch(() => setProfile(EMPTY_REFUND))
  ...
  async function saveProfile() {
    ...
    try {
      ...
      setProfile(updated as RefundOfficeProfile);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <input type="text" className={inputClass} style={readStyle} readOnly value="Refund Office" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-medium" style={{ color: "var(--color-muted-foreground)" }}>
                  Status
      ...
      {/* Refund Office Identity Profile */}
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: "var(--color-muted-foreground)" }}>
              Your identity as Refund Office staff.
            </p>
          </div>
          {!editingProfile && (
            <button
              ...
              style={{ color: "var(--color-primary)" }}
            >
              {profile.staffName || profile.department ? "Edit" : "Set Up"}
            </button>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) => setProfile((p) => ({ ...p, staffName: e.target.value }))}
              />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium" style={{ color: "var(--color-muted-foreground)" }}>
              ...
              <input
                ...
                style={editingProfile ? inputStyle : readStyle}
                readOnly={!editingProfile}
                placeholder="e.g. Finance & Refunds"
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                style={editingProfile ? inputStyle : readStyle}
                readOnly={!editingProfile}
                placeholder="e.g. refunds@platform.com"

...
export default function WorkerProfilePage() {
  ...
  const [profile, setProfile] = useState<WorkerProfile>(EMPTY_WORKER);
  ...
  useEffect(() => {
    ...
    userService
      ...
      .then((p) => setProfile(p as WorkerProfile))
  ...
  async function saveProfile() {
    ...
    try {
      ...
      setProfile(updated as WorkerProfile);
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            {editingProfile && (
              <div className="flex gap-3 pt-1">
                <button
                  onClick={saveProfile}
                  disabled={savingProfile || !profile.professionalTitle.trim()}
                  className="px-4 py-2 rounded-lg text-sm font-medium text-white transition"
                  style={{ backgroundColor: "var(--color-primary)" }}
                >
